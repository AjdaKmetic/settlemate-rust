...
pub fn update_expense(
    ...
    let expense = data.expenses.iter_mut().find(|e| e.id == expense_id).ok_or("Expense not found")?;
...
mod tests {
    ...
    fn test_add_expense() {
        ...
        data.users.push(User::new(1, "Alice", "alice@example.com"));
        let splits = Split::new(vec![(1, 20.0)]);
        ...
        assert_eq!(dto.paid_by, 1);
        assert_eq!(dto.group_id, None);
        assert_eq!(dto.splits.len(), 1);
        assert_eq!(dto.splits[0].user_id, 1);
        assert_eq!(dto.splits[0].amount, 20.0);
    }
    ...
    fn test_update_expense() {
        ...
        let splits = Split::new(vec![(1, 20.0)]);
        let dto = add_expense(&mut data, "Dinner".to_string(), 20.0, 1, None, splits).unwrap();
        let updated_dto = update_expense(&mut data, dto.id, Some("Lunch".to_string()), Some(15.0), None, None, None).unwrap();
        assert_eq!(updated_dto.description, "Lunch");
        assert_eq!(updated_dto.amount, 15.0);
    }
    ...
    fn test_delete_expense() {
        ...
        let splits = Split::new(vec![(1, 20.0)]);
        let dto = add_expense(&mut data, "Dinner".to_string(), 20.0, 1, None, splits).unwrap();
        let result = delete_expense(&mut data, dto.id);
        assert!(result.is_ok());
    }
